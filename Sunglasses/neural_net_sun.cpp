...
double Neuron::alpha = 0.01;   // momentum, multiplier of last deltaWeight, [0.0..1.0]
...
int main()
{
    TrainingData trainData("straightrnd_train1.txt");
    ...
    while (!trainData.isEof()) 
    {
        ...
        if (trainingPass > 100 && myNet.getRecentAverageError() < 0.015)
        {
    ...
    cout << "Testing Data 1" << endl;

    TrainingData trainData1("straightrnd_test1.txt");
    ...
    while (!trainData1.isEof()) 
    {
        ...
        if (abs(resultVals[0] - targetVals[0]) < 0.2)
        {
            accuracy++;
        }
        cout << "accuracy: " << (double)accuracy/(double)trainingPass << endl << endl;
    }

    cout << "Testing Data 2:" << endl;

    TrainingData trainData2("straightrnd_test2.txt");
