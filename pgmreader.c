...
int main(){
  int t ;
  scanf("%d",&t);
  char filename[20][20] = {"an2i","at33","boland","bpm","ch4f","cheyer","choon","danieln","glickman","karyadi","kawamura","kk49","megak","mitchell","night","phoebe","saavik","steffi","sz24","tammo"};
  char sunglasses[20] = "sunglasses";
  char pose[4][20] = {"left","right","up","straight"};
  while(t--)
  {
    IMAGE *new;
    char read[100];
    ...
    if(compstr(sunglasses,read) == 1){
      output = 1;
    }

    /*
    for(l = 0;l < 20 ;l++){
      if(compstr(filename[l],read) == 1){
        output = l;
      }
    }
    */
    ...
    for(;i < new->rows;i++ ){
      for(j = 0;j < new->cols;j++){
        printf("%d ",new->data[i*(new->rows) + j] );
      }
  
    }
    double out = (double)output/20.0;
